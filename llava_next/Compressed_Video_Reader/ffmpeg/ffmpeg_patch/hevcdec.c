...
static void hevc_export_mvs_side_data(HEVCContext *s, AVFrame *frame)
{
    ...
    // Fixed: export 2 vectors per PU (L0 + L1)
    nb_mvs = min_pu_width * min_pu_height * 2;
    if (nb_mvs <= 0)
        return;

    sd = av_frame_new_side_data(frame,
                                AV_FRAME_DATA_MOTION_VECTORS,
                                nb_mvs * sizeof(AVMotionVector));
    if (!sd)
        return;
        ...
        for (x_pu = 0; x_pu < min_pu_width; x_pu++) {
            ...
            // -------- L0 slot --------
            {
                ...
                if (src->pred_flag & PF_L0) {
                    dst->source   = src->ref_idx[0];      // L0: 0..15
                    dst->motion_x = src->mv[0].x;
                    dst->motion_y = src->mv[0].y;
                    dst->src_x    = dst->dst_x + (dst->motion_x >> 2);
                    dst->src_y    = dst->dst_y + (dst->motion_y >> 2);
                    dst->motion_scale = 1;
                } else {
                    dst->source = -1;
                }
            ...
            // -------- L1 slot --------
            {
                ...
                if (src->pred_flag & PF_L1) {
                    dst->source   = src->ref_idx[1] + 16; // L1: 16..31 (use +16 to distinguish)
                    dst->motion_x = src->mv[1].x;
                    dst->motion_y = src->mv[1].y;
                    dst->src_x    = dst->dst_x + (dst->motion_x >> 2);
                    dst->src_y    = dst->dst_y + (dst->motion_y >> 2);
                    dst->motion_scale = 1;
                } else {
