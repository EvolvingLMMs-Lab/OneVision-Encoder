...
// Enable residual-only reconstruction (output 128+residual via modified idct_add)
// Set to 0 to keep original full reconstruction path.
#ifndef CVR_RESIDUAL_ONLY
#define CVR_RESIDUAL_ONLY 1
#endif
...
#if CVR_RESIDUAL_ONLY
    {
        // Residual-only mode: skip intra/inter prediction and deblocking.
        // We pre-fill the MB region with mid-level (1<<(bit_depth-1)) so blocks with zero coeffs remain as 128.
        const int mid = 1 << (h->ps.sps->bit_depth_luma - 1);

        // Fill luma 16x16
        if (PIXEL_SHIFT) {
            for (i = 0; i < 16; i++) {
                uint16_t *row = (uint16_t *)(dest_y + i * linesize);
                for (j = 0; j < 16; j++)
                    row[j] = (uint16_t)mid;
            }
        } else {
            for (i = 0; i < 16; i++)
                memset(dest_y + i * linesize, mid, 16);
        }

        // Fill chroma (if present)
        if ((SIMPLE || !CONFIG_GRAY || !(h->flags & AV_CODEC_FLAG_GRAY)) &&
            h->ps.sps->chroma_format_idc) {
            if (PIXEL_SHIFT) {
                for (i = 0; i < block_h; i++) {
                    uint16_t *row_cb = (uint16_t *)(dest_cb + i * uvlinesize);
                    uint16_t *row_cr = (uint16_t *)(dest_cr + i * uvlinesize);
                    for (j = 0; j < 8; j++) {
                        row_cb[j] = (uint16_t)mid;
                        row_cr[j] = (uint16_t)mid;
                    }
                }
            } else {
                for (i = 0; i < block_h; i++) {
                    memset(dest_cb + i * uvlinesize, mid, 8);
                    memset(dest_cr + i * uvlinesize, mid, 8);
                }
            }
        }

        // Intra PCM has no transform residual; treat as zero-residual.
        if (!SIMPLE && IS_INTRA_PCM(mb_type))
            return;

        // Respect transform_bypass in residual-only mode so bypass paths
        // (used by some inter blocks) still write residuals correctly.
        ...
        if ((SIMPLE || !CONFIG_GRAY || !(h->flags & AV_CODEC_FLAG_GRAY)) &&
            ...
            if (sl->non_zero_count_cache[scan8[CHROMA_DC_BLOCK_INDEX + 0]])
                h->h264dsp.h264_chroma_dc_dequant_idct(
