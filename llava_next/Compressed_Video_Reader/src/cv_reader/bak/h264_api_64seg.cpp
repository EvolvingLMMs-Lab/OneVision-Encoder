...
decode_h264_packet_cb(AVPacket *pkt,
    ...
    while (ret >= 0) {
        ...
        // frame_idx for this decoded frame
                ...
                if (dec_ctx->codec_id == AV_CODEC_ID_HEVC) {
                    ...
                    for (i = 0; i < nb_mvs; i++) {
                        ...
                        if (p_dst_y >= 0 && p_dst_y < height / 4 &&
                            ...
                            // for our HEVC patch, source == 0 is L0 and source > 0 is L1 (we used 16+ref_idx for L1).
                            bool is_L0 = (mv->source == 0 || mv->source < 0);
                            if (is_L0) {
...
static PyObject *decode_h264(AVFormatContext *fmt_ctx, bool with_residual, int max_frames) {
    ...
    // Ensure export_mvs is enabled on the codec context (for H.264 / HEVC / MPEG4)
...
static PyObject *read_video_cb(PyObject *self, PyObject *args) {
    ...
    try {
        // read_video_cb(path, callback, without_residual=0, max_frames=0, frame_ids=None)
        if (!PyArg_ParseTuple(args, "sO|iiO", &path, &callback, &without_residual, &max_frames, &frame_ids_obj)) {
        ...
        if (codec_id == AV_CODEC_ID_H264 || codec_id == AV_CODEC_ID_HEVC || codec_id == AV_CODEC_ID_MPEG4) {
