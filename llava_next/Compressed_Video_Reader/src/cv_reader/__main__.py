...
def main():
    ...
    if len(video_frames) > 0:
        ...
        for frame_idx, frame in enumerate(video_frames):
            print(
                f"\rWriting: {frame_idx + 1}/{len(video_frames)}",
                flush=True,
                end="" if frame_idx + 1 != len(video_frames) else "\n",
            )
            cv2.imwrite(
                os.path.join(args.output, f"{str(frame_idx).rjust(num_digits, '0')}_{frame['pict_type']}_mv_pre.jpg"),
                flow_vis.flow_to_color(frame["motion_vector"][..., :2]),
            )
            cv2.imwrite(
                os.path.join(args.output, f"{str(frame_idx).rjust(num_digits, '0')}_{frame['pict_type']}_mv_post.jpg"),
                flow_vis.flow_to_color(frame["motion_vector"][..., 2:4]),
            )
            cv2.imwrite(
                os.path.join(args.output, f"{str(frame_idx).rjust(num_digits, '0')}_{frame['pict_type']}_res.jpg"),
                frame["residual"],
            )
