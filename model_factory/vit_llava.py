...
class PretrainEncoder(nn.Module):
    ...
    def apply_mask_with_i_p_frames(self, x, list_I, list_P, mask_generator, patch_size, tubelet_size):
        """
        ...
        Args:
            ...
            mask_generator: VjepaMaskGenerator instance
            patch_size: Spatial patch size (height, width)
            tubelet_size: Temporal patch size (number of frames per tube)
        
        Returns:
            masks: Binary mask tensor of shape (b, num_patches) where 1 means keep, 0 means masked
            masked_indices: Indices of masked patches
            kept_indices: Indices of kept patches
        """
        b, c, t, h, w = x.shape
        ...
        # Initialize masks - all ones (keep all) initially
        final_masks = torch.ones((b, total_patches), dtype=torch.float32, device=x.device)
        
        # Process each sample in the batch
        for batch_idx in range(b):
            # Get I and P frame indices for this sample
            i_frames = list_I[batch_idx]  # Shape: (num_i_frames,)
            p_frames = list_P[batch_idx]  # Shape: (num_p_frames,)
            
            # Extract P frames only
            p_frames_tensor = x[batch_idx:batch_idx+1, :, p_frames]  # Shape: (1, c, num_p_frames, h, w)
            
            # Get number of P frames
            num_p_frames = p_frames.shape[0]
            
            # Calculate number of temporal patches for P frames
            p_t_patches = num_p_frames // tubelet_size
            
            # Generate masks only for P frames if we have enough frames
            if p_t_patches > 0:
                # Call mask generator for P frames
                # The mask generator expects the temporal dimension to match its configuration
                # We need to adjust the generator's duration temporarily
                original_duration = mask_generator.duration
                mask_generator.duration = p_t_patches
                
                # Generate masks (returns encoder and predictor masks)
                enc_masks, pred_masks = mask_generator(1)  # Generate for batch size 1
                
                # Restore original duration
                mask_generator.duration = original_duration
                
                # enc_masks contains indices of patches to keep (shape: (1, num_kept_patches))
                # We need to convert this to a full binary mask
                p_frame_mask = torch.zeros((p_t_patches * h_patches * w_patches,), 
                                        dtype=torch.float32, device=x.device)
                p_frame_mask[enc_masks[0]] = 1  # Set kept patches to 1
                
                # Now map P frame patches back to original video timeline
                # Convert P frame indices to temporal patch indices
                p_patch_indices = []
                for p_idx in p_frames:
                    # Find which temporal patch this frame belongs to
                    patch_idx = p_idx // tubelet_size
                    if patch_idx < t_patches:
                        p_patch_indices.append(patch_idx)
                
                # Remove duplicates and sort
                p_patch_indices = sorted(list(set(p_patch_indices)))
                
                # Map P frame mask to full video mask
                for i, p_patch_idx in enumerate(p_patch_indices):
                    if i < p_t_patches:
                        # Get spatial patches for this temporal position
                        start_idx = p_patch_idx * h_patches * w_patches
                        end_idx = start_idx + h_patches * w_patches
                        
                        # Copy mask values from P frame mask
            ...
            # Ensure I frame patches remain unmasked (set to 1)
            i_patch_indices = []
            for i_idx in i_frames:
                patch_idx = i_idx // tubelet_size
                if patch_idx < t_patches:
                    i_patch_indices.append(patch_idx)
            
            # Set all patches corresponding to I frames to 1 (keep)
            for i_patch_idx in set(i_patch_indices):
                start_idx = i_patch_idx * h_patches * w_patches
                end_idx = start_idx + h_patches * w_patches
                final_masks[batch_idx, start_idx:end_idx] = 1
        
        # Get masked and kept indices
        masked_indices = []
        kept_indices = []
        ...
        ids_restore = torch.stack([torch.cat([m, k]) for m, k in zip(masked_indices, kept_indices)], dim=0)
        ids_restore = []
        for m, k in zip(masked_indices, kept_indices):
            id_restore = torch.cat([m, k])          # 拼接
            id_restore= id_restore - 256                    # 所有值减去 256
            id_restore = id_restore[id_restore >= 0]                  # 过滤掉小于 0 的
    ...
    def forward(self, x: torch.Tensor, list_I = None, list_P = None):
        ...
        mask, ids_restore = self.apply_mask_with_i_p_frames(x, list_I, list_P, self.mask_generator, self.patch_size, self.tubelet_size)
        ...
        if mask is not None:
            ...
            mask_frame = mask_frame[:, keep_bool]
        ...
        cls_token = x[:, :1, :]
        x = x[:, 1:, :]
        x_unmask   = x[i_mask].reshape(B, -1, D)   # I帧 (B, Ni, D)
        x_mask = x[p_mask].reshape(B, -1, D) 
...
class PretrainDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.proj_embed = nn.Linear(embed_dim, out_embed_dim, bias=True)
