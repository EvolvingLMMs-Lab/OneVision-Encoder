...
> **Note**: To verify consistency between the packing model and the source model (`vit_preview_v0`), run the conversion script which includes automatic consistency tests for both image and video inputs:
> ```bash
> python model_factory/convert_llava_vit_packing_to_hf.py llava_vit_large_ln /path/to/backbone.pt
> ```
> The script runs `verify_consistency_packing()` and `verify_video_consistency_packing()` functions to compare outputs.
